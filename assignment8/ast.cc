...
//                   Node methods
//
Node::Node() : op_(0), type_(0), name_(""), value_(0), node_() {}
 ...
 * 関数ノード
 */
FuncNode::FuncNode(Node* ret, Node* id, List* list, Node* stat) {
  op_ = OP::FUNC;
  type_ = OP::FUNCNODE;
  node_[0] = ret;
  node_[1] = id;
...
// この関数にでtop_allocをセットしている
void FuncNode::PrintNode(std::ostream &os) {
  // top_allocを初期化する
  token_driver->set_top_alloc_zero();
  os << "( ";
  PrintNum(0, os);
  os << " ";
  PrintNum(1, os);
  ...
  os << ")\n";
  PrintNum(3, os);
  top_alloc = token_driver->get_top_alloc();
  // std::cerr << top_alloc << endl;
}
/*
 *  FundNode END
 */
...
int DeclarationList::set_all_offset() {
  ...
  return count;
  // return token_driver->get_top_alloc();
}